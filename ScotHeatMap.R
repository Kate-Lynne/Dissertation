...
# great - NAME_2  gives us 32 names at county level: this will be useful later

#################
# So, we have found that scot_shape has a list of 32 area names in $NAME_2;
# Now, if we can create an array - let's call it 'linkdata' -- which associates
# those  32 names with 32 heatmap values, we will be able to use that
...
heatvals <- sample(0:1,replace=TRUE,len)
...
# have a look at it - see that it links the area names directly to heatval values
linkdata
...
# first thing we need to do, it turns out, is to produce a 'data frame' version of scot_shapefile
...
# so, let's  set heatvals[26] to 100
...
## you should now see Borders as a light area (heatval 100) with the rest dark (heatvals 0 or 1) 
--------------------------------
